...
def extract_answer(query, contexts):
    ...
    for doc in contexts:
        ...
        for idx, sentence in enumerate(sentences):
            ...
            # 2. Strict Subject Definition Check for "what is X" style queries
            # Check if sentence actually defines/answers the query's primary subject
            # Primary subject pattern matching:
            kw_regex_parts = [r'\b' + re.escape(kw) + r"(s|'s|ies)?" + r'\b' for kw in keywords]
            kw_pattern = r'(' + '|'.join(kw_regex_parts) + r')'

            # Patterns:
            # Pattern A: Sentence starts with (a/an/the)? Subject (definition|is|are|means|refers to|incorporated|created|defined|was|were)
            ...
            # Pattern B: "X is a Y" or "is a X" or "a X is" anywhere in sentence
            ...
            # Pattern C: Subject definition header, e.g. "Corporation definition," or "Company:"
            ...
            if not is_match:
                # If score is very high (>0.75) and contains all keywords, check if it has explanatory verbs
            ...
            if is_match:
                ...
                if re.search(pattern_start, s_lower) or re.search(pattern_def_header, s_lower):
                    score += 10.0
...
if __name__ == "__main__":
    ...
    out_file = os.path.join(PROJECT_ROOT, "pattern_out.txt")
    ...
    print("Done writing pattern_out.txt")
